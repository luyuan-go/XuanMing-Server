...
CREATE TABLE IF NOT EXISTS `accounts` (
    ...
    `player_no`   BIGINT UNSIGNED       NULL COMMENT '角色编号(展示专用,禁作身份键/外键/幂等键;绑定角色实体——今 player_id 即角色身份,卖角色过户时随角色走、值不变,故一账号建 N 角色 = N 个编号;NULL=待补号,login 补号任务异步分配,player-no-and-login-surge.md §3.3/§3.6.1)',
