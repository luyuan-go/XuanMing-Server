...
| INC-20260804-001 | 2026-08-04 | P0 | availability | 修复实施中(未关闭) | mode=local 全链断裂：Model B 落地后新增的 owner 权威接线在 legacy 面**成片漏接**，玩家依次卡在进大厅被踢 / 进大厅确认不了 / PVE 对局判 FAILED / 进副本确认不了 / 退副本被拒。共 8 处（含 1 处修复引入的自锁死），前 6 处已由真实客户端端到端验证。**第⑦处「对局正常结算不释放 owner」非 local 专属——Agones+legacy 与标准 Model B 生产同样缺失**（全仓 owner 释放此前只接了登出/判弃/saga 中止三条异常路径），已按同一手法修 Model B 侧但**真集群未验证** | [事故报告](2026-08-04-p0-local-legacy-owner-wiring-gaps.md) / [待验证清单](2026-08-04-p0-local-legacy-owner-wiring-PENDING-VERIFICATION.md) |
