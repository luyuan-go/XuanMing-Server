...
> **已落码(2026-08-12 拍板并实现)**:对局已切成**准备 / 战斗 / 结算**三段,见
> [`match-phase-lifecycle.md`](match-phase-lifecycle.md) §7/§8。对本节流程的影响只有一处:
> 下面这条链路的**起点被推迟**——退场通知改在 `max(终局投递握手完成, 结算窗口到点)` 才发出,
> 且 Agones 回收一并推迟(否则玩家会在 DS 消失之后才收到回流 RPC = "结算完回不去大厅")。
> **链路本身、消息与顺序都没变**,仍是下图这一条出口,没有新建退场路径。
