...
> 生效口径：[`CLAUDE.md §3.1`](../../CLAUDE.md) 的“首次生产上线日期”是唯一兼容闸。日期未填写时，
> 本文是首发目标架构，允许在非生产环境用一个完整批次重建二进制、schema 与数据；日期填写后，本文的
> 混版、迁移和不停服要求全部成为硬门禁。正确性、安全性、数据不丢和玩家不卡死等要求始终生效。

---

## 0. 结论先行

Pandora 后端的**生产目标就是不停服更新**,分两层保障,缺一不可:
...
首次生产上线日期填写后，滚动更新期间「新版本写了新字段 → 旧版本副本读到并回写」是常态，所以
**兼容性是滚动更新的硬约束**，不是可选项。日期未填写时的有意 breaking 必须按 `CLAUDE.md §3.1`
刷新整批读写者、数据与生成物，不能留下混合快照。

---

## 2. 数据层:Redis 二进制 pb 的 schema 演进规则(生产兼容模式硬规则)
...
- ✅ **删字段**:首次生产上线日期填写后，先 `reserved <num>;` + `reserved "<name>";` 并注释原因，**永不复用编号**；日期未填写时按 `CLAUDE.md §3.1` 的整批刷新闭环处理。
...
首次生产上线日期填写后，破坏性变更**只能**走「加新字段 + 双写 + 灰度 + 下线旧字段」的多版本迁移，
不允许原地改；日期未填写时允许原地重整，但必须刷新所有受影响数据与消费者。
