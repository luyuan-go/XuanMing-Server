...
func TestDiscoverLevel(t *testing.T) {
	...
	if len(d.columns) != 19 || d.columns[0].header != "ID" ||
...
func sampleGrid() [][]string {
	return [][]string{
		{"ID", "关卡名称", "关卡资源", "GameMode类", "关卡类别", "禁止ui快捷键开关", "匹配列表显示", "队伍人数", "是否能退出", "玩法模式", "入口模式", "对局方数", "经验归属方式", "对局时长", "队伍人数下限", "计分模式", "段位池", "准备时长", "结算时长"},
		{"", "", "", "", "", "0:不禁止", "0:不显示(默认)", "一方人数(team_size)", "0:不可退出(默认)", "撮合池 game_mode", "1:撮合", "PVP 通常为 2", "1:最后一击(默认)", "0=不限时", "只对直进生效", "1:不计分", "算哪一份段位分", "", ""},
		{"", "", "", "", "", "1:禁止(默认)", "1:显示", "本地1v1填1;正式5v5填5", "1:可主动退出", "", "2:直进 3:两种都开放", "PVE 通常为 1", "2:全队共享", "", "0=无下限", "2:按Elo算段位", "计分=2 必填,否则留空", "", ""},
		{},
		{"1", "登录", "/Game/Level/Login/Lvl_Login.Lvl_Login", "", "1", "", "0", "", "0", "", "", "", "", "", "", "", "", "0", "0"},
		{"2", "选角", "/Game/Level/RoleSelect/Lvl_RoleSelect.Lvl_RoleSelect", "", "2", "", "0", "", "0", "", "", "", "", "", "", "", "", "0", "0"},
		{"6", "MOBA战斗", "/Game/Test/Level/MobaLevel.MobaLevel", "/Script/Pandora.PandoraBattleGameMode", "4", "", "1", "3", "0", "5v5_ranked", "1", "2", "1", "0", "", "2", "5v5_ranked", "0", "0"},
		{"7", "松林镇副本", "/Game/Test/Level/SonglinTown.SonglinTown", "/Script/Pandora.PandoraPveGameMode", "4", "", "1", "1", "1", "pve_coop", "2", "1", "2", "600", "", "1", "", "0", "0"},
