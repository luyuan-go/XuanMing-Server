...
func TestDiscoverLevel(t *testing.T) {
	...
	if len(d.columns) != 20 || d.columns[0].header != "ID" ||
		...
		d.columns[15].header != "计分模式" || d.columns[16].header != "段位池" ||
		d.columns[17].header != "准备时长" || d.columns[18].header != "结算时长" ||
		d.columns[19].header != "准备模式" {
...
func sampleGrid() [][]string {
	return [][]string{
		{"ID", "关卡名称", "关卡资源", "GameMode类", "关卡类别", "禁止ui快捷键开关", "匹配列表显示", "队伍人数", "是否能退出", "玩法模式", "入口模式", "对局方数", "经验归属方式", "对局时长", "队伍人数下限", "计分模式", "段位池", "准备时长", "结算时长", "准备模式"},
		{"", "", "", "", "", "0:不禁止", "0:不显示(默认)", "一方人数(team_size)", "0:不可退出(默认)", "撮合池 game_mode", "1:撮合", "PVP 通常为 2", "1:最后一击(默认)", "0=不限时", "只对直进生效", "1:不计分", "算哪一份段位分", "", "", "1:匹配前准备"},
		{"", "", "", "", "", "1:禁止(默认)", "1:显示", "本地1v1填1;正式5v5填5", "1:可主动退出", "", "2:直进 3:两种都开放", "PVE 通常为 1", "2:全队共享", "", "0=无下限", "2:按Elo算段位", "计分=2 必填,否则留空", "", "", "2:成功后确认(留空同)"},
		{},
		{"1", "登录", "/Game/Level/Login/Lvl_Login.Lvl_Login", "", "1", "", "0", "", "0", "", "", "", "", "", "", "", "", "0", "0", ""},
		{"2", "选角", "/Game/Level/RoleSelect/Lvl_RoleSelect.Lvl_RoleSelect", "", "2", "", "0", "", "0", "", "", "", "", "", "", "", "", "0", "0", ""},
		{"6", "MOBA战斗", "/Game/Test/Level/MobaLevel.MobaLevel", "/Script/Pandora.PandoraBattleGameMode", "4", "", "1", "3", "0", "5v5_ranked", "1", "2", "1", "0", "", "2", "5v5_ranked", "0", "0", "2"},
		{"7", "松林镇副本", "/Game/Test/Level/SonglinTown.SonglinTown", "/Script/Pandora.PandoraPveGameMode", "4", "", "1", "1", "1", "pve_coop", "2", "1", "2", "600", "", "1", "", "0", "0", "1"},
...
func TestBuildLevelHappyPath(t *testing.T) {
	...
	if r6.Id != 6 || r6.Category != configpb.LevelCategory_LEVEL_CATEGORY_BATTLE ||
		...
		r6.RatingPool != "5v5_ranked" ||
		r6.ReadyMode != configpb.LevelReadyMode_LEVEL_READY_MODE_POST_CONFIRM {
	...
	if r7.TeamSize != 1 || !r7.AllowExit || r7.GameMode != "pve_coop" ||
		...
		r7.RatingMode != configpb.LevelRatingMode_LEVEL_RATING_MODE_NONE || r7.RatingPool != "" ||
		r7.ReadyMode != configpb.LevelReadyMode_LEVEL_READY_MODE_PRE_READY {
		t.Fatalf("id7 新增列解析错误: %+v", r7)
	}
	// 准备模式留空 = 落 0(UNSPECIFIED),服务端按「撮合成功后确认」处理:
	// 这正是本列对存量表零改动的依据,断言它别哪天被加上 (excel_default) 悄悄变掉。
	if data.Rows[0].ReadyMode != configpb.LevelReadyMode_LEVEL_READY_MODE_UNSPECIFIED {
		t.Fatalf("id1 准备模式留空应为 UNSPECIFIED: %+v", data.Rows[0])
	}
